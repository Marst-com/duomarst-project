...
```
duomarst-project/
├── frontend/        HTML은 껍데기만, 실제 UI는 js/main.js가 전부 렌더링함 (빌드 도구 없음, 그냥 vanilla JS)
│   ├── index.html      <body data-page="home"><div id="root"></div></body> 형태의 최소 껍데기
│   ├── history.html     data-page="history"
│   ├── members.html     data-page="members"
│   ├── booking.html     data-page="booking"
│   ├── contact.html     data-page="contact"
│   ├── login.html       data-page="login"
│   ├── dashboard.html   data-page="dashboard"
│   ├── css/style.css   다크/화이트 테마
│   ├── assets/          로고 · 파비콘
│   └── js/main.js      PAGE_BODIES에 페이지별 HTML을 문자열로 담아두고,
│                        data-page 값을 읽어서 #root에 조립해 넣음.
│                        나머지(테마 토글, 폼, 예약 로직 등)도 전부 여기서 처리.
│
└── backend/         Firebase, 이메일 발송, 세션 등 민감한 로직 전부 여기
    ├── server.js
    ├── routes/{auth,contact,user,booking}.js
...
`frontend/js/main.js`의 `PAGE_BODIES` 객체에서 해당 페이지 키(`home`, `history`, `members`, `booking`, `contact`, `login`, `dashboard`)를 찾아 그 템플릿 문자열을 수정하면 됨. HTML 파일 자체는 건드릴 필요 없음.

## 예약(booking) 기능

- 상담 가능 요일/시간은 `backend/routes/booking.js` 상단 `SCHEDULE` 객체에 정의되어 있음 (현재: 월/목 19:00~21:00, 화/금 16:00~21:00, 20분 단위)
- 예약은 Firestore `bookings` 컬렉션에 `날짜_시간`을 문서 ID로 저장 — 같은 시간에 동시에 두 명이 신청해도 하나만 성공하도록 원자적으로 처리됨
- **Firebase 콘솔 → Firestore Database에서 데이터베이스를 미리 만들어둬야 함** (없으면 예약 API가 오류 남)
- 예약 가능 시간대를 바꾸려면 `SCHEDULE` 값만 수정하면 됨
